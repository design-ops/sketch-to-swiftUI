...
const processLayerStyles = async (layerStyles, filepath, outputFolder, modifiers) => {
    ...
    layerStyles.forEach(async (style) => {
        ...
        let currentModifier = {modifier: null, score: 0};
        for (const radiusModifier of modifiers.radius) {
          const modName = getModifierNameWithoutModifier(radiusModifier);
          const score = matchScore(style.name, modName)
          if (currentModifier.score < score) {
            currentModifier.modifier = radiusModifier;
            currentModifier.score = score
          }
        }
        if (currentModifier.modifier != null) {
          addOptionalProperty("cornerRadius", data.style, cornerRadius(currentModifier.modifier) )
        }
      ...
      for(let i=0;i<borders.length;i++){
        ...
        if (border.fillType == 0 && border.isEnabled == true) { // Solid Color
            ...
            if (cornerRadius && cornerRadius > 0) {
              borderArray.push(`RoundedRectangle(cornerRadius: ${cornerRadius}).stroke(${borderColor}, lineWidth: ${border.thickness})`)
            } else {
        ...
        if (border.fillType == 1 && border.isEnabled == true) { // Gradients
          ...
          if (border.gradient.gradientType == 0) { // Linear Gradients
            ...
            if (cornerRadius && cornerRadius > 0) {
              ...
              borderArray.push(`RoundedRectangle(cornerRadius: ${cornerRadius}).stroke(${gradient}, lineWidth: ${border.thickness})`)
            } else {
