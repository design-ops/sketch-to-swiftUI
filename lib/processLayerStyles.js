import { addOptionalProperty, colorToRGBA, doRound, prepareAssetBundleForPDF, prepareAssetBundleForPNG, prepareAssetBundleForJPG } from './processShared.js'
import { matchScore } from "./identifierMatcher.js"
import fs from 'fs'
import sketchToSvg from 'sketch-to-svg'
...
export const processLayerStyles = async (layerStyles, filepath, outputFolder, modifiers) => {
    let swiftUIStyles = []
    let uiKitProperties = []
    ...
    layerStyles.forEach(async (style) => {
      ...
      // Check to see if there are any shadows.
      // It's important to add these after we have clipped the shape of the layer,
      // since otherwise the shadow will not be shown due to it being clipped.
      ...
      if (shadows !== undefined && shadows.length > 0) {
          ...
          addOptionalProperty("shadow", data.style, shadowArray)
      }
...
const prepareAssetBundle = (imageName, assetBundleFolder) => {
  ...
  if (extension === "pdf") {
    ...
    prepareAssetBundleForJPG(assetBundleFolder, name)
  } else {
