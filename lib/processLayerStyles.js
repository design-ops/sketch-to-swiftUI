...
const processLayerStyles = async (layerStyles, filepath, outputFolder, modifiers) => {
    ...
    layerStyles.forEach(async (style) => {
        ...
        addOptionalProperty("clipped", data.style, layerClipped(layerStyle) )
        addOptionalProperty("shadow", data.style, layerShadow(layerStyle) )
        addOptionalProperty("padding", data.style, layerPadding(layerStyle) )
...
const layerClipped = (layerStyles) => {
  let shadows = layerStyles.shadows
  if (shadows !== undefined && shadows.length > 0) {
    for(let i=0;i<shadows.length;i++){
      let shadow = shadows[i]
...
const layerPadding = (layerStyles) => {
  let shadows = layerStyles.shadows
  if (shadows !== undefined && shadows.length > 0) {
    ...
    for(let i=0;i<shadows.length;i++){
      let shadow = shadows[i]
      if (shadow.isEnabled == true) {
        // Figure out what the padding is on either side
        // If there are mutliple shadows, we only keep the highest paddings
        topPadding = Math.max(topPadding, shadow.blurRadius - doRound(shadow.offsetY))
        bottomPadding = Math.max(bottomPadding, shadow.blurRadius + doRound(shadow.offsetY))
        leftPadding = Math.max(leftPadding, shadow.blurRadius - doRound(shadow.offsetX))
        rightPadding = Math.max(rightPadding, shadow.blurRadius + doRound(shadow.offsetX))
      }
    }
    paddingArray.push(`EdgeInsets(top: ${topPadding}, leading: ${leftPadding}, bottom: ${bottomPadding}, trailing: ${rightPadding})`)
    return paddingArray
  }
}
