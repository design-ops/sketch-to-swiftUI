  ...
  let properties = []
    ...
    for (let index = 0; index < textStyles.length; ++index) {
        ...
        const propertyData = {
          name: style.name,
          properties: [
            `.textColor(UIColor(red: ${color.red}, green: ${color.green}, blue: ${color.blue}, alpha: ${color.alpha}))`,
            `.kerning(${encodedAttributes.kerning})`,
            `.font(UIFont(name: "${fontAttributes.name}", size: ${fontAttributes.size})!)` // using ! to unwrap the font. Should we do this? IMHO the font not being there is a programmer error
          ]
        }
    ...
    return { text: {styles, properties}, fonts: fonts }
}
