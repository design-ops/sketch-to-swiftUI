  ...
  let uiKitProperties = []
    ...
    for (let index = 0; index < textStyles.length; ++index) {
        ...
        const propertyData = {
          name: style.name,
          properties: [
            { name: "textColor", value: `UIColor(red: ${color.red}, green: ${color.green}, blue: ${color.blue}, alpha: ${color.alpha})`},
            // using ! to unwrap the font. Should we do this? IMHO the font not being there is a programmer error
            { name: "font", value: `UIFont(name: "${fontAttributes.name}", size: ${fontAttributes.size})!`},
          ]
        }

        if (encodedAttributes.kerning) {
          propertyData.properties.push({ name: "kerning", value: encodedAttributes.kerning})
        }
