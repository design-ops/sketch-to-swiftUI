...
async function processSketchFile(sketch, filepath, outputFolder) {
    ...
    textStyles.text.styles.sort(sortByName)
    textStyles.text.properties.sort(sortByName)
    console.log(`    📝 Processed ${textStyles.text.styles.length} Text Styles!\n`)
    ...
    styles.styles = merge(textStyles.text.styles, layers.styles)
    styles.styles.sort(sortByName)
    styles.uiKitProps = mergeProperties(textStyles.text.properties, layers.properties)
