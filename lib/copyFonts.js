 ...
 */
export const copyFont = async (name, outputFolder) => { 
 ...
 */
export const copyFonts = async (names, embeddedFontData, outputFolder) => {

    // Make sure the output folder exists
    let folder = outputFolder + "/assets/fonts/"
    await mkdirPromise(folder, { recursive: true })
...
async function performCopy(name, embeddedFontData, allFonts, folder) {
    ...
    for (let fontReferenceIndex = 0; fontReferenceIndex < embeddedFontData.usedFontReferences.length; fontReferenceIndex++) {
        ...
        if(fontRef.postscriptNames.includes(name) && fontRef.fontData) {
            ...
            await copyFilePromise(origin, destination)
            return {
                family: name, 
                path: destination,
    ...
    const fileExists = await existsPromise(finalPath)
    if (fileExists) {
        return returnedFont
    }
    ...
    if (!font.isFromCollection) {
        await copyFilePromise(font.fullPath, finalPath)
        return returnedFont
    }

    // We can sync copy the font file across, everything else is a system command, so promises yay.
    await copyFilePromise(font.fullPath, tempFontPath)
...
class Font {
    ...
    static async allFrom(filename) {
        ...
        try {
            const font = await open(filename)
            if (!font.fonts) {
                return [new Font(filename, font)]
            }
    ...
    static async allFonts() {
        const extract = async function (path) {
            let entries = await readdirPromise(path)
            let fonts = []
            for (let index = 0; index < entries.length; ++index) {
                ...
                const stat = await statPromise(absolute)
                if (stat.isDirectory()) {
